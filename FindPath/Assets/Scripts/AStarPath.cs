...
public class AStarPath : MonoBehaviour
{
    ...
    void FindAStarPath(APos pos)
    {
        ...
        if(nextPos == null)
        {
            ...
            {
                if(openList[i].F < minValueFInOpenList)
                {
                    minValueFInOpenList = openList[i].F;
                    nextPosInOpenList = openList[i];
                }
            }

            nextPos = nextPosInOpenList;
        }

        var parentPos = searchMap[nextPos.x, nextPos.z].parent;
        //Debug.Log("Nextpos:" + nextPos.x + "|" + nextPos.z);
        //Debug.Log("Parent:" + parentPos.x + "|" + parentPos.z);
    ...
    void ShowPath(APos pos)
    {
        ...
        if (parentPos != start)
        {
            var findPathItem_clone = Instantiate(findPathItem, new Vector3(parentPos.x, findPathItem.position.y, parentPos.z), Quaternion.identity);
